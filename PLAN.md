...
2. **リポジトリを public 化** — **実施済み (2026-06-11)**。public 化後に Release asset の
   自動ダウンロードが実際に通ることを検証済み。topics も 11 個に拡充
   (yolo / onnx / self-driving-car / bdd100k / object-detection / lane-detection 追加)
3. **重みの GitHub Releases 配布 + 初回自動ダウンロード** — **実施済み (2026-06-11)**。
   Release v0.1.0 に `adas_yolov8n_bdd100k.pt` / `traffic_light_state.onnx` を配置。
   `adas_perception/weights.py` が config 中の既知モデル参照を sha256 検証付きで自動取得
   (TwinLiteNet は upstream MIT repo から)。`ADASPerceptionPipeline.__init__` に組み込み済み。
   注意: private リポジトリのままでは認証なし URL が 404 になるため、自動 DL の実効性は
   public 化後に発生する (検証済み挙動)
...
1. **README ヒーロー GIF** — **実施済み (2026-06-11)**。Pexels CC0 dashcam (1080p) に
   `configs/demo_video_full.yaml` (post-NMS 検出 + TwinLiteNet lane + Kalman tracking +
   ground 融合距離 + ONNX TL state) + planning overlay を適用し、冒頭 6.5 秒を
   `assets/demo_hero.gif` (600px/8fps/5.8MB)、全 11 秒を `assets/demo_hero.mp4` (960px/2.9MB)
   として README 冒頭に配置
...
## Official train 70k 再学習 (2026-06-11 着手、学習は一時中断中)

「official BDD100K train split の配置」のブロッカーが解消した。公式ミラー
(dl.cv.ethz.ch) は消滅、doc.bdd100k.com も到達不能だが、**Kaggle ミラー
`a7madmostafa/bdd100k-yolo` (5.7GB, YOLO 形式変換済み, 2025-10 更新, CC-BY-SA-4.0)**
から train 70,000 枚 + ラベルを取得できた (このマシンの kaggle CLI は認証済み)。

- 配置: `data/bdd100k_yolo_kaggle/` (train 70k / val 10k / test、YOLO txt)
- クラス: 2018 ラベル系の 10 クラス。`data_adas.yaml` で det_20 準拠名に rename
  (person→pedestrian, bike→bicycle, motor→motorcycle) して既存 class_groups と整合
- リーク確認済み: Kaggle train と手元 official val 10k の画像名重複は 0
- 学習: yolov8n.pt (COCO) から epochs=10, imgsz=1024, batch=auto(11), GPU
  → `runs/official_train/yolov8n_70k_1024`
- **状態: 2026-06-11 に epoch 1 途中で中断** (実測 2.5 it/s、6,364 it/epoch =
  約 42 分/epoch、10 epoch ≈ 8 時間)。checkpoint 未保存のため再開はゼロから:

  ```bash
  .venv/bin/yolo detect train \
    data=data/bdd100k_yolo_kaggle/data_adas.yaml \
    model=yolov8n.pt epochs=10 imgsz=1024 batch=-1 device=0 workers=8 \
    project=runs/official_train name=yolov8n_70k_1024
  ```

  時間短縮の選択肢: imgsz=640 (約3倍速、小物体精度は低下) / epochs=5 / 夜間実行
- 学習完了後の手順: odd 5,000 macro F1 評価 (`evaluate_bdd100k.py`、従来プロトコル)
  → per-kind threshold 再チューニング → 現行 0.6355 (単一)/0.6763 (WBF) と比較
  → 上回れば Release v0.1.0 の重み差し替え + README 数値更新
- 次段候補: YOLOv8s 同条件 (精度上限狙い、約2倍時間)
